...
const SummaryItem: React.FC<{ icon: React.ReactNode; label: string; value: string | number }> = ({ icon, label, value }) => (
  <div className="flex items-start gap-4 p-3 bg-surface-element rounded-lg">
    ...
    <div><p className="text-sm text-text-secondary">{label}</p><p className="text-lg font-medium text-text-primary">{value}</p></div>
  </div>
);

export const CampaignSummaryModal: React.FC = () => {
  const { campaignId } = useParams({ from: '/campaigns/$campaignId' });
  const { campaignSummary, setShowCampaignSummaryModal, selectedRecipientIndices, clearRecipientSelection } = useAppStore();
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 md:p-8">
      ...
      <motion.div className="relative w-full h-full max-w-6xl bg-surface-card border border-borders-primary rounded-card shadow-card flex flex-col" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}>
        <div className="p-4 border-b border-borders-primary flex justify-between items-center flex-shrink-0">
          ...
          <button onClick={handleClose} className="p-1 rounded-full text-text-secondary hover:bg-surface-element-hover hover:text-text-primary transition-colors flex-shrink-0"><X size={20} /></button>
        </div>
        <div className="flex-grow p-4 grid grid-cols-1 lg:grid-cols-3 gap-4 min-h-0">
          <div className="lg:col-span-1 flex flex-col gap-4">
            ...
            <SummaryItem icon={<FileStack size={20} />} label="Total Attachment Size" value={formatBytes(campaignSummary.total_attachment_size_bytes)} />
            <div className="mt-auto pt-4">
              ...
              <div className="flex items-center justify-end gap-3">
                ...
                <Button variant="primary" onClick={handleConfirmSend} disabled={!hasRecipientsToSend}><Send size={16} />Confirm & Send</Button>
              </div>
            </div>
          </div>
          <div className="lg:col-span-2 flex flex-col min-h-0">
            ...
            <div className="p-3 bg-surface-element rounded-lg mb-4">
              <p className="text-sm text-text-primary"><strong>Subject:</strong> {campaignSummary.preview_subject}</p>
            </div>
            <div className="flex-grow min-h-0">
              <iframe srcDoc={campaignSummary.preview_body} title="Email Preview" className="w-full h-full bg-white border border-borders-primary rounded-lg" sandbox="allow-same-origin" />
            </div>
          </div>
        </div>
      </motion.div>
